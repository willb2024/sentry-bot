...
async function sendCallerMenu(ctx: any, tgId: string, isEdit = false) {
    ...
    `Powered by <b>Self-Learning AI</b> (Rule-Based Safety Audits + Self-Training ML Model). Sentry has been trained on over 10 million+ trades to provide highly accurate predictions and constantly retrains on live market outcomes to adapt.\n\n` +
    ...
    `• <b>Calibrated (ML Trained):</b> Generated by our self-learning AI model trained on over 10 million+ historical trades and breakout patterns. Highly reliable and constantly improving.\n` +
...
bot.on("text", async (ctx, next) => {
    ...
    if (text.toLowerCase() === '/cancel' || text.toLowerCase() === 'cancel') {
        const keysToClear = [
            ...
            `state:enter_ref:${telegramId}`, `state:edit_slippage:${telegramId}`, `state:set_pin:${telegramId}`
        ];
        await redis.del(...keysToClear);
        await ctx.replyWithHTML(`✅ <b>Action Cancelled.</b> Return to menu with /start`);
        return;
    }
