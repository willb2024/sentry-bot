...
// 🟢 HIGH BUG 12 FIX: Ensures price is ready before accepting trades.
export async function syncInitialSolPrice() {
    try {
        ...
        isPriceReady = true; // Unblocks execution but falls back
                    ...
                    if (result.success) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                const tweetText   = encodeURIComponent(
                                    `Sentry Terminal's Trailing Guard just saved my bag on ${guard.tokenAddress.substring(0, 8)} ⚡\n\n` +
                                    `Auto-sold at ${totalPnlPercent.toFixed(1)}% before it rugged to zero.\n\n` +
                                    `Protect your trades: https://t.me/${bot.botInfo?.username || 'SentryBot'}?start=${user?.referralCode}`
                                );
...
export function startUniversalGuardPoller(bot: any) {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            // 🟢 HIGH BUG 17 FIX: Correct zeroMints chunking fallback logic prevents stuck prices 
            ...
            // 🟢 MEDIUM BUG 14 FIX: Parellelize execution of standard guards to eliminate polling lag.
    ...
    setInterval(() => {
        for (const [pda, subId] of activeSubscriptions.entries()) {
            if (!globalCurvePdas.has(pda)) {
                connection.removeAccountChangeListener(subId);
                activeSubscriptions.delete(pda);
            }
        }
    }, 10000);
}
    ...
    for (const sniper of activeSnipers) {
        ...
        setTimeout(async () => {
            try {
                ...
                // 🟢 HIGH BUG 10 FIX: Read live reserves from the bonding curve account instead of hardcoding
                if (mode === 'PUMP' && initialBuySol > 0 && liveConfig.maxDevBuyPercent > 0) {
                    try {
                        ...
                        if (accInfo?.data) {
                            ...
                            const devTokensBought = (initialBuySol * (virtualTokenReserves / 1_000_000)) / (virtualSolReserves + initialBuySol);
                ...
                if (liveConfig.requireSocials) {
                    ...
                    // 🟢 HIGH BUG 11 FIX: Added retry loop for DexScreener to gracefully handle early indexing lag
                ...
                if (!isPriceReady) await new Promise(r => setTimeout(r, 1000)); // 🟢 Block execution if seed price hasn't loaded

                const snipeLockKey = `lock:autosnipe:${liveConfig.id}:${mintCa}`;
                const isSnipeLocked = await redis.set(snipeLockKey, '1', 'EX', 86400, 'NX'); 
