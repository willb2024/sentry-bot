...
export async function loadSimulationState(telegramId: string) {
    ...
    const state = await prisma.simState.findUnique({ 
        where: { userId: user.id }, 
        include: { trades: { orderBy: { createdAt: 'desc' }, take: 500 } } 
    });
    if (!state) return;
...
export async function generateSimCallerAlert(telegramId: string, filters: { minScore: number; maxAgeMins: number; minPctChange: number; maxPctChange: number; minLiquidity: number; minVolume24h: number; blockMev: boolean; }): Promise<any> {
    try {
        ...
        if (hotRaw) {
            ...
            if (matching.length > 0) {
                ...
                if (bestMatch) return { mint: bestMatch.mint, symbol: bestMatch.symbol, score: bestMatch.totalScore ?? bestMatch.score, reasons: bestMatch.reasons || [], ageMins: bestMatch.ageMins, priceChangeM5: bestMatch.priceChangeM5 || 0, mevRisk: bestMatch.breakdown?.mevRisk ?? 0, liquidity: bestMatch.liquidity, volume: bestMatch.volume, isReshow };
            }
        }
    } catch (e) { }
    ...
    for (let i = 0; i < poolSize; i++) {
        ...
        candidates.push({ mint: realToken.mint, symbol: realToken.symbol, totalScore: simScore, reasons: computeTokenScore(stats).reasons, ageMins: stats.ageMins, priceChangeM5: stats.priceChangeM5, liquidity: stats.liquidity, volume: stats.volume24h, mevRisk: stats.isRug ? -100 : 0 });
    }
...
async function runSimAutoSnipeLoop(telegramId: string, bot: any) {
    ...
    while (await redis.get(`sim:autosnipe:${telegramId}`) === 'true' && await isSimulationActive(telegramId)) {
        ...
        let tokenCA = '', stats: any, simScore = 0, liqUsd = 0, cachedSolUsdPrice = 150;

        // 🟢 IMPORT: Ensure live price is available for liquidity calculations
        const { cachedSolUsdPrice: liveSolPrice } = await import('./grpc.service.js');
        cachedSolUsdPrice = liveSolPrice || 150;
        ...
        if (alert && alert.mint) {
            ...
            stats = { ageMins: Math.floor(Math.random() * 60) + 1, volume: Math.random() * 50000 + 5000, liquidity: Math.random() * 20000 + 2000, priceChangeM5: Math.random() * 50 + 5, hasSocials: true, isRug: false };
            liqUsd = stats.liquidity;
        ...
        // 🟢 APPLY DYNAMIC SIZING ENGINE IN SIMULATION
        let snipeAmount = config.amountSol; // Legacy fallback

        if (config.enableDynamicScaling) {
            const normalizedScore = Math.min(100, Math.max(10, simScore)) / 100;
            const exponent = config.scaleExponent || 2.0;
            const convictionMultiplier = Math.pow(normalizedScore, exponent);

            let calculatedSize = config.baseRiskUnitSol * convictionMultiplier * config.maxRiskMultiplier;
            ...
            // Budget Protection
            const budgetRemaining = (config.maxBudgetSol || Infinity) - currentSpend;
        ...
        // 🟢 Execute snipe with dynamic `snipeAmount`
        const buyRes = await simExecuteSnipe(telegramId, tokenCA, snipeAmount);
        if (!buyRes.success) {
        ...
        const buyMsg = buildAuditTrailMessage(tokenCA, simScore, stats, buyRes.volumeSpent, slPercent, config?.autoTakeProfitPercent ? tpPercent : 'OFF', true) + `\n\n🔗 <a href="https://solscan.io/tx/${buyRes.signature}">View on Solscan</a>`;
...
async function sendSimPnlCard(telegramId: string, bot: any, tokenAddress: string, amountInSol: number, pnlPercent: number, slPercent: number, entryPriceSol: number, tokensBought: number) {
    ...
    try {
        ...
        await bot.telegram.sendPhoto(telegramId, { source: imageBuffer }, { caption: captionText, parse_mode: 'HTML', reply_markup: twitterBtn });
    } catch (e: any) {
...
setInterval(async () => {
    try {
        const simStates = await prisma.simState.findMany({ where: { active: true }, select: { user: { select: { telegramId: true } } } });
